...
`evals/cases/sales_v1.jsonl` хранит исторический baseline. Набор
`evals/cases/sales_v2.jsonl` добавляет проверки мягкого тона, уважения к самооценке
пользователя, доказательности гипотез и веток осведомлённости о тест-драйве.

Запуск актуального набора:

```bash
.venv/bin/python -m evals.run_sales_eval run \
  --dataset evals/cases/sales_v2.jsonl \
  --run-id sales-v2
```

В каждом наборе один JSON-объект находится на отдельной строке. `transcript` содержит
только предыдущие реплики; текущее сообщение лежит в `user_message`.
...
## Business regression 2026-07-29

`evals/cases/sales_v4.jsonl` сохраняет все 36 сценариев `sales_v3` и добавляет семь
кейсов из обратной связи по реальному диалогу. Они проверяют:

- грамматически естественный русский язык без случайных повторов;
- отсутствие неуклюжих конструкций с параллельными числовыми целями;
- отсутствие повторного подтверждения уже названной цели;
- конкретную пользу тест-драйва для найденного участка проекта;
- порядок «персональная польза -> длительность и цена»;
- прямой ответ о 1000 рублях, когда пользователь сам спросил цену;
- отсутствие повторного pitch и цены для уже знакомого пользователя.

В hard checks добавлены `no_adjacent_duplicate_words`,
`no_awkward_parallel_numeric_goal` и сценарный `ordered_mentions`. Judge дополнительно
считает две отдельные метрики: `Russian Language Quality` и
`Personalized Value Before Price`. Правило порядка не применяется как причина скрыть
цену при прямом вопросе пользователя и учитывает пользу, уже объяснённую в предыдущей
реплике.

Для сравнения prompt-версий dataset, target-модель, judge-модель и версия метрик должны
быть одинаковыми. Сначала запускается baseline, затем candidate; менять критерии между
этими двумя прогонами нельзя.
